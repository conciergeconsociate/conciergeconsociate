...
function applyTemplate(htmlBody: string, title: string): string {
  const currentYear = new Date().getFullYear().toString();
  return EMAIL_TEMPLATE
    .replace("{{ .EmailBody }}", htmlBody)
...
export async function sendCampaignEmail(params: { subject: string; html: string; to: string[] | string }) {
  ...
  const templatedHtml = applyTemplate(html, subject);
  return supabase.functions.invoke("campaigns", { body: { subject, html: templatedHtml, to } });
}
