...
import { sendCampaignEmail } from "@/lib/emailCampaigns";
...
export default function Admin() {
  ...
  const [isDeletingInvited, setIsDeletingInvited] = useState<string | null>(null);
  // System Emails state
  const [systemEmails, setSystemEmails] = useState<any[]>([]);
  const [loadingSystemEmails, setLoadingSystemEmails] = useState(false);
  const [emailDialogOpen, setEmailDialogOpen] = useState(false);
  const [editingEmail, setEditingEmail] = useState<any | null>(null);
  const [emailForm, setEmailForm] = useState({ name: "", subject: "", html_body: "" });
  ...
  const [isSendingBookingBulk, setIsSendingBookingBulk] = useState(false);
  const [isSendingVaBulk, setIsSendingVaBulk] = useState(false);
  const [isSendingNewsletterBulk, setIsSendingNewsletterBulk] = useState(false);
  const [isSendingSingle, setIsSendingSingle] = useState(false);
  const [isSendingUsersBulk, setIsSendingUsersBulk] = useState(false);
  const [isSendingContactsBulk, setIsSendingContactsBulk] = useState(false);
...
const [editingPlan, setEditingPlan] = useState<any | null>(null);
  ...
  const [planForm, setPlanForm] = useState({ name: "", duration: "30", price: "", coverImage: "", benefits: "", showOnWebsite: true });
  ...
  const loadVenues = async () => {
    ...
    try {
      const { data } = await supabase
        .from("venues")
        .select("id,created_at,name,short_description,detailed_description,category,niche,address,weekday_hours,sunday_hours,map_url,images,hidden,featured")
        .order("created_at", { ascending: false })
        .limit(100);
  ...
  const toggleVenueFeatured = async (v: Venue) => { try { await supabase.from("venues").update({ featured: !v.featured }).eq("id", v.id); await loadVenues(); } catch {} };
  ...
  const [isDeletingFaq, setIsDeletingFaq] = useState<string | null>(null);
  const [isDeletingVoucher, setIsDeletingVoucher] = useState<string | null>(null);
  const [isSubmittingTransfer, setIsSubmittingTransfer] = useState(false);
  const [isLoadingVisits, setIsLoadingVisits] = useState(false);
  const [isDeletingVenue, setIsDeletingVenue] = useState<string | null>(null);
  ...
  const [isActivatingUser, setIsActivatingUser] = useState<string | null>(null);
  const [isMarkingPasswordUpdated, setIsMarkingPasswordUpdated] = useState<string | null>(null);
  ...
  const markPasswordUpdated = async (id: string) => {
    try { setIsMarkingPasswordUpdated(id); await supabase.from("admin_added_users").update({ status: "password_updated" }).eq("id", id); toast({ title: "Marked as password updated" }); await loadInvitedUsers(); } catch (e:any) { toast({ title: "Error", description: e?.message ?? "Unknown error", variant: "destructive" }); } finally { setIsMarkingPasswordUpdated(null); }
  };
  const activateInvitedUser = async (invite: any) => {
    try {
      setIsActivatingUser(invite.id);
    ...
    } catch (e:any) { toast({ title: "Error", description: e?.message ?? "Unknown error", variant: "destructive" }); } finally { setIsActivatingUser(null); }
  };

  const deleteInvitedUser = async (invite: any) => {
    if (!confirm("Are you sure you want to delete this invitation?")) return;
    try {
      setIsDeletingInvited(invite.id);
      
      // If user has a profile, delete via edge function to ensure full cleanup
      if (invite.profile_id) {
        const { error } = await supabase.functions.invoke('delete-user', {
          body: { user_id: invite.profile_id }
        });
        if (error) throw error;
      } else {
        // Otherwise just delete from admin_added_users
        const { error } = await supabase.from("admin_added_users").delete().eq("id", invite.id);
        if (error) throw error;
      }
  ...
  // CRUD helpers: System Emails
  const loadSystemEmails = async () => {
    setLoadingSystemEmails(true);
    try {
      const { data } = await supabase.from("system_emails").select("*").order("name");
      setSystemEmails(data || []);
    } catch {}
    setLoadingSystemEmails(false);
  };
  const openEditEmail = (e: any) => {
    setEditingEmail(e);
    setEmailForm({ name: e.name || "", subject: e.subject || "", html_body: e.html_body || "" });
  ...
  const saveSystemEmail = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        .from("system_emails")
        .update({ name: emailForm.name, subject: emailForm.subject, html_body: emailForm.html_body, updated_at: new Date().toISOString() })
        .eq("id", editingEmail.id);
      if (error) throw new Error(error.message);
      toast({ title: "Updated", description: "Email template updated" });
      setEmailDialogOpen(false);
      setEditingEmail(null);
      await loadSystemEmails();
    } catch (e: any) {
      toast({ title: "Error", description: e?.message ?? "Unknown error", variant: "destructive" });
    } finally {
  ...
  useEffect(() => { if (activeTab === "others") { loadFaqs(); if (othersTab === "vouchers") loadVouchers(); if (othersTab === "pages") loadPagesList(); if (othersTab === "system_emails") loadSystemEmails(); } }, [activeTab, othersTab]);
    ...
    (async () => {
      try {
        ...
        const [
          ...
          supabase.from("website_visits").select("id", { count: "estimated", head: true }),
          ...
          supabase.from("contact_submissions").select("id, created_at, name, email, phone, subject, message, status").order("created_at", { ascending: false }).limit(10),
          supabase.from("concierge_requests").select("id, created_at, name, email, phone, service, message, status").order("created_at", { ascending: false }).limit(10),
          supabase.from("newsletter_subscriptions").select("id, created_at, email, source, path, referrer, user_agent, confirmed").order("created_at", { ascending: false }).limit(10),
          supabase.from("profiles").select("id, created_at, email, full_name, role").eq("role", "admin").order("created_at", { ascending: false }).limit(10),
          userId ? supabase.from("profiles").select("id, created_at, email, full_name, phone, role").eq("id", userId).maybeSingle() : Promise.resolve({ data: null })
        ]);
        ...
        if (isMounted) {
          ...
          setCurrentAdminProfile(meRes.data || null);
  ...
  if (isMobile) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 text-center bg-background">
        <LayoutDashboard className="w-16 h-16 mb-4 text-primary" />
        <h1 className="text-2xl font-bold mb-2">Desktop Required</h1>
        <p className="text-muted-foreground max-w-md">
          The Admin Panel is optimized for desktop use. Please access this page from a larger screen for the best experience.
        </p>
      ...
      <Sidebar collapsible="icon">
        <SidebarHeader>
          <div className="flex items-center gap-2 px-2 py-2 text-sidebar-foreground">
            <img src="/brand-logo.svg" alt="Admin Panel" className="h-8 w-8 transition-all group-data-[collapsible=icon]:h-6 group-data-[collapsible=icon]:w-6" />
            <span className="truncate font-bold text-xl group-data-[collapsible=icon]:hidden">Admin Panel</span>
          </div>
        </SidebarHeader>
        <SidebarContent>
          <SidebarGroup>
                                  ...
                                  <AlertDialog>
                                    ...
                                    <AlertDialogContent>
                                      ...
                                      <AlertDialogFooter>
                                        ...
                                        <AlertDialogAction disabled={isDeletingService === s.id} onClick={async (e) => { e.preventDefault(); try { setIsDeletingService(s.id); await handleDeleteService(s.id); } finally { setIsDeletingService(null); } }}>{isDeletingService === s.id ? "Deleting..." : "Delete"}</AlertDialogAction>
                                      </AlertDialogFooter>
                                    </AlertDialogContent>
                                  </AlertDialog>
                                </div>
                ...
                <Dialog open={serviceDialogOpen} onOpenChange={setServiceDialogOpen}>
                  <DialogContent>
                    ...
                    <DialogFooter>
                      ...
                      <Button disabled={isSavingService} onClick={async () => { try { setIsSavingService(true); await handleSaveService(); } finally { setIsSavingService(false); } }}>{isSavingService ? "Saving..." : (editingService ? "Save changes" : "Create service")}</Button>
                    </DialogFooter>
                  </DialogContent>
              </Dialog>
                          ...
                          <div key={v.id} className="relative group overflow-hidden rounded-xl ring-1 ring-border">
                            ...
                            <div className="absolute inset-x-0 bottom-0 p-4 flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <Button variant="ghost" size="icon" className="text-white hover:bg-white/20" onClick={() => toggleVenueFeatured(v)}>
                                  <Star className={`h-4 w-4 ${v.featured ? "fill-yellow-400 text-yellow-400" : ""}`} />
                                </Button>
                                ...
                                <AlertDialog>
                                  ...
                                  <AlertDialogContent>
                                    ...
                                    <AlertDialogFooter>
                                      <AlertDialogCancel>Cancel</AlertDialogCancel>
                                      <AlertDialogAction disabled={isDeletingVenue === v.id} onClick={async (e) => { e.preventDefault(); try { setIsDeletingVenue(v.id); await deleteVenue(v.id); } finally { setIsDeletingVenue(null); } }}>{isDeletingVenue === v.id ? "Deleting..." : "Delete"}</AlertDialogAction>
                                    </AlertDialogFooter>
                                  </AlertDialogContent>
                                </AlertDialog>
                              </div>
                ...
                <Dialog open={venueDialogOpen} onOpenChange={setVenueDialogOpen}>
                  <DialogContent>
                    ...
                    <DialogFooter>
                      ...
                      <Button disabled={isSavingVenue} onClick={async () => { try { setIsSavingVenue(true); await handleSaveVenue(); } finally { setIsSavingVenue(false); } }}>{isSavingVenue ? "Saving..." : (editingVenue ? "Save changes" : "Create venue")}</Button>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              </div>
                              ...
                              {invitedUsers
                                ...
                                .map((u) => (
                                  <TableRow key={u.id}>
                                    ...
                                    <TableCell>
                                      <div className="flex gap-2">
                                        <Button variant="outline" size="sm" onClick={() => markPasswordUpdated(u.id)} disabled={u.status !== "invited" || isMarkingPasswordUpdated === u.id}>{isMarkingPasswordUpdated === u.id ? "Updating..." : "Mark Password Updated"}</Button>
                                        <Button variant="default" size="sm" onClick={() => activateInvitedUser(u)} disabled={u.status === "activated" || isActivatingUser === u.id}>{isActivatingUser === u.id ? "Activating..." : "Activate"}</Button>
                                        <Button variant="destructive" size="sm" onClick={() => deleteInvitedUser(u)} disabled={isDeletingInvited === u.id}>{isDeletingInvited === u.id ? "Deleting..." : "Delete"}</Button>
                                      </div>
                                    </TableCell>
                                  </TableRow>
                                ))}
                      ...
                      <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="h-64 mb-12">
                            <ReactQuill theme="snow" value={bulkBookingBody} onChange={setBulkBookingBody} className="h-full" />
                          </div>
                        ...
                        <DialogFooter>
                          ...
                          <Button disabled={isSendingBookingBulk} onClick={async () => {
                            try {
                              setIsSendingBookingBulk(true);
                              const recipientList = (selectedBookings.size ? bookings.filter(b => selectedBookings.has(b.id)) : bookings);
                              const recipientEmails = recipientList.map(b => b.email);

                              if (recipientEmails.length === 0) {
                                toast({ title: "No recipients", variant: "destructive" });
                                setIsSendingBookingBulk(false);
                                return;
                              }

                              await sendCampaignEmail({ to: recipientEmails, subject: bulkBookingSubject, html: bulkBookingBody });

                              const { data: campaign } = await supabase.from("email_campaigns").insert({ subject: bulkBookingSubject, body: bulkBookingBody, status: "sent" }).select("id").maybeSingle();
                              if (campaign?.id) {
                                const recipientsData = recipientList.map(r => ({ campaign_id: campaign.id, email: r.email, name: r.name || null }));
                                await supabase.from("email_campaign_recipients").insert(recipientsData);
                              }

                              toast({ title: "Email sent", description: `Sent to ${recipientEmails.length} recipients.` });
                          ...
                          }}>{isSendingBookingBulk ? "Sending..." : "Send Email"}</Button>
                        </DialogFooter>
                      </DialogContent>
                    </Dialog>
                    ...
                    <Dialog open={vaBulkEmailOpen} onOpenChange={setVaBulkEmailOpen}>
                      <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
                        ...
                        <div className="space-y-3">
                          ...
                          <Label>Message</Label>
                          <div className="h-64 mb-12">
                            <ReactQuill theme="snow" value={bulkVaBody} onChange={setBulkVaBody} className="h-full" />
                          </div>
                        ...
                        <DialogFooter>
                          ...
                          <Button disabled={isSendingVaBulk} onClick={async () => {
                            try {
                              setIsSendingVaBulk(true);
                              const recipientList = (selectedVa.size ? vaRequests.filter(r => selectedVa.has(r.id)) : vaRequests);
                              const recipientEmails = recipientList.map(r => r.email);

                              if (recipientEmails.length === 0) {
                                toast({ title: "No recipients", variant: "destructive" });
                                setIsSendingVaBulk(false);
                                return;
                              }

                              await sendCampaignEmail({ to: recipientEmails, subject: bulkVaSubject, html: bulkVaBody });

                              const { data: campaign } = await supabase.from("email_campaigns").insert({ subject: bulkVaSubject, body: bulkVaBody, status: "sent" }).select("id").maybeSingle();
                              if (campaign?.id) {
                                const recipientsData = recipientList.map(r => ({ campaign_id: campaign.id, email: r.email, name: r.name || null }));
                                await supabase.from("email_campaign_recipients").insert(recipientsData);
                              }
                          ...
                          }}>{isSendingVaBulk ? "Sending..." : "Send Email"}</Button>
                        </DialogFooter>
                      </DialogContent>
                    </Dialog>
                  </TabsContent>
                ...
                <Dialog open={membershipDialogOpen} onOpenChange={setMembershipDialogOpen}>
                  <DialogContent>
                    ...
                    <DialogFooter>
                      ...
                      <Button disabled={isSavingMembership} onClick={async () => { try { setIsSavingMembership(true); await saveMembershipSettings(); } finally { setIsSavingMembership(false); } }} className="bg-primary text-primary-foreground hover:bg-primary/90">{isSavingMembership ? "Saving..." : "Save changes"}</Button>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              </div>
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <AlertDialog>
                                    ...
                                    <AlertDialogContent>
                                      ...
                                      <AlertDialogFooter>
                                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                                        <AlertDialogAction disabled={isDeletingBlog === b.id} onClick={async (e) => { e.preventDefault(); try { setIsDeletingBlog(b.id); await deleteBlog(b.id); } finally { setIsDeletingBlog(null); } }}>{isDeletingBlog === b.id ? "Deleting..." : "Delete"}</AlertDialogAction>
                                      </AlertDialogFooter>
                                    </AlertDialogContent>
                                  </AlertDialog>
                                </div>
                ...
                <Dialog open={blogDialogOpen} onOpenChange={setBlogDialogOpen}>
                  <DialogContent>
                    ...
                    <DialogFooter>
                      ...
                      <Button disabled={isSavingBlog} onClick={async () => { try { setIsSavingBlog(true); await handleSaveBlog(); } finally { setIsSavingBlog(false); } }}>{isSavingBlog ? "Saving..." : (editingBlog ? "Save changes" : "Create blog")}</Button>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              </div>
                                  ...
                                  <div key={t.id} className="rounded-xl ring-1 ring-border p-4 space-y-2">
                                    <div className="font-semibold flex items-center justify-between">
                                      ...
                                      <div className="flex gap-2">
                                        ...
                                        <Button size="sm" variant="destructive" disabled={isDeletingTestimonial === t.id} onClick={async () => { try { setIsDeletingTestimonial(t.id); await deleteTestimonial(t.id); } finally { setIsDeletingTestimonial(null); } }}>{isDeletingTestimonial === t.id ? "Deleting..." : "Delete"}</Button>
                                      </div>
                                    </div>
                        ...
                        <Dialog open={testimonialDialogOpen} onOpenChange={setTestimonialDialogOpen}>
                          <DialogContent>
                            ...
                            <DialogFooter>
                              ...
                              <Button disabled={isSavingTestimonial} onClick={async () => { try { setIsSavingTestimonial(true); await saveTestimonial(); } finally { setIsSavingTestimonial(false); } }}>{isSavingTestimonial ? "Saving..." : (editingTestimonial ? "Save Changes" : "Save")}</Button>
                            </DialogFooter>
                          </DialogContent>
                        </Dialog>
                      </TabsContent>
                      <TabsContent value="contact">
                        <Card>
                          <CardContent className="pt-6 space-y-4">
                            ...
                            <div className="flex justify-end"><Button disabled={isSavingContactInfo} onClick={async ()=> {
                              try {
                              ...
                              } catch (e: any) { toast({ title: "Error", description: e?.message ?? "Unknown error", variant: "destructive" }); } finally { setIsSavingContactInfo(false); }
                            }}>{isSavingContactInfo ? "Saving..." : "Save"}</Button></div>
                          </CardContent>
                        </Card>
                      </TabsContent>
                      ...
                      <TabsContent value="social">
                        <Card>
                          <CardContent className="pt-6 space-y-2">
                            ...
                            <div className="flex justify-end"><Button disabled={isSavingSocialLinks} onClick={async ()=> {
                              try {
                              ...
                              } catch (e:any) { toast({ title: "Error", description: e?.message ?? "Unknown error", variant: "destructive" }); } finally { setIsSavingSocialLinks(false); }
                            }}>{isSavingSocialLinks ? "Saving..." : "Save"}</Button></div>
                          </CardContent>
                        </Card>
                      </TabsContent>
                    </Tabs>
                              ...
                              <div key={i} className="rounded-xl ring-1 ring-border p-4 space-y-3">
                                <div className="flex items-center justify-between">
                                  <div>
                                    ...
                                    <div className="text-sm text-muted-foreground">{p.duration} Days • ₦{p.price}</div>
                                  </div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                  ...
                                  <Button size="sm" variant="secondary" onClick={()=> { setEditingPlan(p); setPlanDialogOpen(true); setPlanForm({ name: p.name || "", duration: String(p.duration || "30"), price: String(p.price ?? ""), coverImage: p.cover_image || "", benefits: Array.isArray(p.benefits) ? (p.benefits as string[]).join("\n") : "", showOnWebsite: !!p.show_on_website }); }}>Edit</Button>
                                </div>
                              </div>
                            ))}
                          </div>
                    ...
                    <Dialog open={planDialogOpen} onOpenChange={(v)=>{ setPlanDialogOpen(v); if (!v) { setEditingPlan(null); } }}>
                      <DialogContent>
                        ...
                        <DialogFooter>
                          ...
                          <Button disabled={isSavingPlan} onClick={async ()=>{
                            try {
                              ...
                              if (editingPlan) {
                                const { error } = await supabase
                                  .from("membership_plans")
                                  .update({ name: planForm.name, duration: Number(planForm.duration), price: Number(planForm.price), cover_image: coverUrl || null, benefits: (planForm.benefits || "").split("\n").filter(Boolean), show_on_website: !!planForm.showOnWebsite })
                                  .eq("id", editingPlan.id);
                                if (error) throw new Error(error.message);
                              } else {
                                const { error } = await supabase
                                  .from("membership_plans")
                                  .insert({ name: planForm.name, duration: Number(planForm.duration), price: Number(planForm.price), cover_image: coverUrl || null, benefits: (planForm.benefits || "").split("\n").filter(Boolean), show_on_website: !!planForm.showOnWebsite });
                                if (error) throw new Error(error.message);
                              }
                              const { data } = await supabase
                                .from("membership_plans")
                            ...
                            } catch (e:any) { toast({ title: "Error", description: e?.message ?? "Unknown error", variant: "destructive" }); } finally { setIsSavingPlan(false); }
                          }}>{isSavingPlan ? "Saving..." : "Save"}</Button>
                        </DialogFooter>
                      </DialogContent>
                    </Dialog>
                            ...
                            <TableBody>
                              {planUsers.map((u)=> (
                                <TableRow key={u.id}>
                                  ...
                                  <TableCell>
                                    <Button size="sm" variant="destructive" disabled={revokingMembershipId === u.id} onClick={async ()=> {
                                      try {
                                        setRevokingMembershipId(u.id);
                                        await revokeMembership(u.id, selectedPlanForUsers!.id);
                                      } finally {
                                        setRevokingMembershipId(null);
                                      }
                                    }}>{revokingMembershipId === u.id ? "Revoking..." : "Revoke"}</Button>
                                  </TableCell>
                                </TableRow>
                              ))}
                            </TableBody>
                    ...
                    <Dialog open={assignDialogOpen} onOpenChange={setAssignDialogOpen}>
                      <DialogContent>
                        ...
                        <DialogFooter>
                          ...
                          <Button disabled={isAssigningPlan} onClick={async () => {
                            try {
                          ...
                          }}>{isAssigningPlan ? "Assigning..." : "Assign"}</Button>
                        </DialogFooter>
                      </DialogContent>
                    </Dialog>
                  </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <Tabs value={othersTab} onValueChange={setOthersTab}>
                      <TabsList>
                        ...
                        <TabsTrigger value="system_emails">System Emails</TabsTrigger>
                      </TabsList>
                      <TabsContent value="faq">
                                  ...
                                  <div key={f.id} className="rounded-xl ring-1 ring-border p-4">
                                    <div className="flex items-center justify-between">
                                      ...
                                      <div className="flex gap-2">
                                        ...
                                        <Button size="sm" variant="destructive" disabled={isDeletingFaq === f.id} onClick={async () => { try { setIsDeletingFaq(f.id); await deleteFaq(f.id); } finally { setIsDeletingFaq(null); } }}>{isDeletingFaq === f.id ? "Deleting..." : "Delete"}</Button>
                                      </div>
                                    </div>
                        ...
                        <Dialog open={faqDialogOpen} onOpenChange={setFaqDialogOpen}>
                          ...
                          <Button disabled={isSavingFaq} onClick={async () => { try { setIsSavingFaq(true); await saveFaq(); } finally { setIsSavingFaq(false); } }}>{isSavingFaq ? "Saving..." : (editingFaq ? "Save Changes" : "Save")}</Button>
                        </DialogFooter>
                          </DialogContent>
                        </Dialog>
                      </TabsContent>
                                ...
                                {vouchers.map((v) => (
                                  <div key={v.id} className="grid grid-cols-2 md:grid-cols-7 gap-2 text-sm items-center">
                                    ...
                                    <div className="flex gap-2 justify-end">
                                      ...
                                      <Button size="sm" variant="destructive" disabled={isDeletingVoucher === v.id} onClick={async () => { try { setIsDeletingVoucher(v.id); await deleteVoucher(v.id); } finally { setIsDeletingVoucher(null); } }}>{isDeletingVoucher === v.id ? "Deleting..." : "Delete"}</Button>
                                    </div>
                                  </div>
                                ))}
                              </div>
                        ...
                        <Dialog open={voucherDialogOpen} onOpenChange={setVoucherDialogOpen}>
                          <DialogContent>
                            ...
                            <DialogFooter>
                              ...
                              <Button disabled={isSavingVoucher} onClick={async () => { try { setIsSavingVoucher(true); await saveVoucher(); } finally { setIsSavingVoucher(false); } }}>{isSavingVoucher ? "Saving..." : (editingVoucher ? "Save Changes" : "Save")}</Button>
                            </DialogFooter>
                          </DialogContent>
                        </Dialog>
                      </TabsContent>
                      ...
                      <TabsContent value="system_emails">
                        <Card>
                          <CardContent className="pt-6">
                            {loadingSystemEmails ? (
                              <div className="space-y-3">
                              ...
                              <div className="text-center py-8 text-muted-foreground">No system emails found.</div>
                            ) : (
                              <div className="space-y-3">
                                {systemEmails.map((email) => (
                                  <div key={email.id} className="rounded-xl ring-1 ring-border p-4">
                                    <div className="flex items-center justify-between">
                                      <div>
                                        <div className="font-semibold">{email.name}</div>
                                        <div className="text-sm text-muted-foreground">Subject: {email.subject}</div>
                                      </div>
                                      <Button size="sm" variant="outline" onClick={() => openEditEmail(email)}>Edit</Button>
                                    </div>
                                  </div>
                                ))}
                              </div>
                          ...
                          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="space-y-2">
                                <Label>Body</Label>
                                <div className="h-96">
                                  <ReactQuill theme="snow" value={emailForm.html_body} onChange={(v) => setEmailForm({ ...emailForm, html_body: v })} className="h-full pb-12" />
                                </div>
                              </div>
                            </div>
                            <DialogFooter className="mt-6">
                              <Button variant="outline" onClick={() => setEmailDialogOpen(false)}>Cancel</Button>
                              <Button disabled={isSavingEmail} onClick={saveSystemEmail}>{isSavingEmail ? "Saving..." : "Save Changes"}</Button>
                            </DialogFooter>
                          </DialogContent>
                        </Dialog>
                      </TabsContent>
                ...
                <Dialog open={editProfileOpen} onOpenChange={setEditProfileOpen}>
                  <DialogContent>
                    ...
                    <DialogFooter>
                      ...
                      <Button disabled={isUpdatingProfile} onClick={async ()=>{ try { setIsUpdatingProfile(true); await supabase.from("profiles").update({ full_name: profileForm.full_name || null, phone: profileForm.phone || null }).eq("id", userId); toast({ title: "Profile updated" }); setEditProfileOpen(false); } catch (e:any) { toast({ title: "Error", description: e?.message ?? "Unknown error", variant: "destructive" }); } finally { setIsUpdatingProfile(false); } }}>{isUpdatingProfile ? "Saving..." : "Save"}</Button>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              </div>
                ...
                <Dialog open={transferDialogOpen} onOpenChange={setTransferDialogOpen}>
                  <DialogContent>
                    ...
                    <DialogFooter>
                      ...
                      <Button disabled={isSubmittingTransfer} onClick={async () => { try { setIsSubmittingTransfer(true); await submitTransfer(); } finally { setIsSubmittingTransfer(false); } }}>{isSubmittingTransfer ? "Submitting..." : "Submit"}</Button>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              </div>
            ...
            <Dialog open={visitsDialogOpen} onOpenChange={setVisitsDialogOpen}>
              <DialogContent className="max-w-3xl">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button disabled={isLoadingVisits} onClick={async () => { try { setIsLoadingVisits(true); await loadVisitsAggregated(); } finally { setIsLoadingVisits(false); } }}>{isLoadingVisits ? "Loading..." : "Apply Filters"}</Button>
            ...
            <Dialog open={bulkEmailOpen} onOpenChange={setBulkEmailOpen}>
              <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
                ...
                <div className="space-y-3">
                  ...
                  <Label>Message</Label>
                  <div className="h-64 mb-12">
                    <ReactQuill theme="snow" value={bulkBody} onChange={setBulkBody} className="h-full" />
                  </div>
                  <p className="text-sm text-muted-foreground pt-10">Recipients: {selectedContacts.size || contacts.length} {selectedContacts.size ? "(selected)" : "(all)"}</p>
                </div>
                <DialogFooter>
                  <Button variant="outline" onClick={() => setBulkEmailOpen(false)}>Cancel</Button>
                  <Button disabled={isSendingContactsBulk} onClick={async () => {
                    try {
                      setIsSendingContactsBulk(true);
                      const recipients = (selectedContacts.size ? contacts.filter(c => selectedContacts.has(c.id)) : contacts).map(c => c.email);
                      if (recipients.length === 0) { toast({ title: "No recipients", variant: "destructive" }); setIsSendingContactsBulk(false); return; }
                      
                      await sendCampaignEmail({ to: recipients, subject: bulkSubject, html: bulkBody });
                      
                      await supabase.from("email_campaigns").insert({ subject: bulkSubject, body: bulkBody, status: "sent" });
                      
                      toast({ title: "Email sent", description: `Sent to ${recipients.length} recipients.` });
                      ...
                      setIsSendingContactsBulk(false);
                    }
                  }}>{isSendingContactsBulk ? "Sending..." : "Send Email"}</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
            ...
            <Dialog open={bulkUsersOpen} onOpenChange={setBulkUsersOpen}>
              <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
                ...
                <div className="space-y-3">
                  ...
                  <Label>Message</Label>
                  <div className="h-64 mb-12">
                    <ReactQuill theme="snow" value={bulkUsersBody} onChange={setBulkUsersBody} className="h-full" />
                  </div>
                ...
                <DialogFooter>
                  ...
                  <Button disabled={isSendingUsersBulk} onClick={async () => {
                    try {
                      setIsSendingUsersBulk(true);
                      ...
                      if (recipients.length === 0) { toast({ title: "No recipients", variant: "destructive" }); setIsSendingUsersBulk(false); return; }

                      await sendCampaignEmail({ to: recipients, subject: bulkUsersSubject, html: bulkUsersBody });

                      await supabase
                        .from("email_campaigns")
                        .insert({ subject: bulkUsersSubject, body: bulkUsersBody, status: "sent" });
                      ...
                      setIsSendingUsersBulk(false);
                    }
                  }}>{isSendingUsersBulk ? "Sending..." : "Send Email"}</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
            ...
            <Dialog open={roleConfirmOpen} onOpenChange={setRoleConfirmOpen}>
              <DialogContent>
                ...
                <DialogFooter>
                  ...
                  <Button disabled={!!isUpdatingRole} onClick={async () => {
                    if (!roleConfirmTarget) return;
                    try {
                      setIsUpdatingRole(roleConfirmTarget.id);
                      await updateUserRole(roleConfirmTarget.id, roleConfirmTarget.nextRole);
                      setRoleConfirmOpen(false);
                    } finally {
                      setIsUpdatingRole(null);
                    }
                  }}>{isUpdatingRole === roleConfirmTarget?.id ? "Updating..." : "Confirm"}</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
            ...
            <Dialog open={newUserOpen} onOpenChange={setNewUserOpen}>
              <DialogContent>
                ...
                <DialogFooter>
                  ...
                  <Button disabled={isCreatingUser} onClick={async ()=>{ try {
                    setIsCreatingUser(true);
                    // Generate a secure password with at least one uppercase, one lowercase, one number, and one symbol
                    const chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*";
                    let temp = "";
                    const array = new Uint32Array(12);
                    window.crypto.getRandomValues(array);
                    for (let i = 0; i < 12; i++) {
                      temp += chars[array[i] % chars.length];
                    }
                    // Ensure complexity
                    if (!/[A-Z]/.test(temp)) temp += "A";
                    if (!/[a-z]/.test(temp)) temp += "a";
                    if (!/[0-9]/.test(temp)) temp += "1";
                    if (!/[!@#$%^&*]/.test(temp)) temp += "!";
                    ...
                    const createdUserId = inviteData.user?.user?.id;
                    if (!createdUserId) throw new Error("No user ID returned from invite function");
                    ...
                    if (!profileCheck) {
                       const { error: profileError } = await supabase.from("profiles").insert({
                          ...
                          full_name: newUserForm.full_name,
                          phone: newUserForm.phone,
                          role: newUserForm.role || 'member',
                          created_at: new Date().toISOString()
                       });
                       if (profileError) {
                          console.error("Profile fallback creation failed:", profileError);
                    ...
                    toast({ title: "User created", description: `Temporary password: ${temp}` });
                    await loadInvitedUsers();
                    setNewUserOpen(false);
                    setNewUserForm({ email: "", full_name: "", phone: "", role: "member" }); } catch (e:any) { toast({ title: "Error", description: e?.message ?? "Unknown error", variant: "destructive" }); } finally { setIsCreatingUser(false); } }}>{isCreatingUser ? "Creating..." : "Create"}</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
            {/* Profile Edit Dialog */}
            <Dialog open={profileEditOpen} onOpenChange={setProfileEditOpen}>
              <DialogContent>
                ...
                <DialogFooter>
                  ...
                  <Button disabled={isUpdatingProfile} onClick={async ()=>{
                    try {
                  ...
                  }}>{isUpdatingProfile ? "Saving..." : "Save"}</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
            ...
            <Dialog open={singleEmailOpen} onOpenChange={setSingleEmailOpen}>
              <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
                ...
                <div className="space-y-3">
                  ...
                  <Label>Message</Label>
                  <div className="mb-8">
                    <ReactQuill theme="snow" value={singleEmailBody} onChange={setSingleEmailBody} />
                  </div>
                </div>
                <DialogFooter>
                  <Button variant="outline" onClick={() => setSingleEmailOpen(false)}>Cancel</Button>
                  <Button disabled={isSendingSingle} onClick={async () => {
                    try {
                      setIsSendingSingle(true);
                      if (!singleEmailRecipient?.email) { toast({ title: "No email", description: "This user has no email.", variant: "destructive" }); setIsSendingSingle(false); return; }
                      
                      const { data: emailData, error: emailError } = await sendCampaignEmail({ to: singleEmailRecipient.email, subject: singleEmailSubject, html: singleEmailBody });
                      if (emailError || (emailData && !emailData.ok)) {
                        throw new Error(emailError?.message || "Failed to send email via provider.");
                      }

                      const { data: campaign } = await supabase
                        .from("email_campaigns")
                        .insert({ subject: singleEmailSubject, body: singleEmailBody, status: "sent" })
                        .select("id")
                        .maybeSingle();

                      if (campaign?.id) {
                        await supabase.from("email_campaign_recipients").insert({ campaign_id: campaign.id, email: singleEmailRecipient.email, name: singleEmailRecipient.full_name });
                      }

                      toast({ title: "Email sent", description: "Recipient added." });
                  ...
                  }}>{isSendingSingle ? "Sending..." : "Send Email"}</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
            ...
            <Dialog open={newsletterBulkEmailOpen} onOpenChange={setNewsletterBulkEmailOpen}>
              <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
                ...
                <div className="space-y-3">
                  ...
                  <Label>Message</Label>
                  <div className="mb-8">
                    <ReactQuill theme="snow" value={newsletterBulkBody} onChange={setNewsletterBulkBody} />
                  </div>
                  <p className="text-sm text-muted-foreground pt-10">Recipients: {selectedNewsletter.size || newsletterSubs.length} {selectedNewsletter.size ? "(selected)" : "(all)"}</p>
                </div>
                <DialogFooter>
                  <Button variant="outline" onClick={() => setNewsletterBulkEmailOpen(false)}>Cancel</Button>
                  <Button disabled={isSendingNewsletterBulk} onClick={async () => {
                    try {
                      setIsSendingNewsletterBulk(true);
                      const recipientList = (selectedNewsletter.size ? newsletterSubs.filter(n => selectedNewsletter.has(n.id)) : newsletterSubs);
                      const recipientEmails = recipientList.map(n => n.email);

                      if (recipientEmails.length === 0) { toast({ title: "No recipients", variant: "destructive" }); setIsSendingNewsletterBulk(false); return; }

                      await sendCampaignEmail({ to: recipientEmails, subject: newsletterBulkSubject, html: newsletterBulkBody });

                      const { data: campaign } = await supabase.from("email_campaigns").insert({ subject: newsletterBulkSubject, body: newsletterBulkBody, status: "sent" }).select("id").maybeSingle();
                      ...
                      setIsSendingNewsletterBulk(false);
                    }
                  }}>{isSendingNewsletterBulk ? "Sending..." : "Send Email"}</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
            {/* Delete User Confirmation */}
            <AlertDialog open={deleteUserDialogOpen} onOpenChange={setDeleteUserDialogOpen}>
              <AlertDialogContent>
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction disabled={isDeletingUser} onClick={async (e) => {
                    e.preventDefault();
                    try {
                      setIsDeletingUser(true);
                      if (!userToDelete?.id) return;
                      
                      // Use edge function for full deletion (Auth + Profile + Admin Added Users)
                  ...
                  }}>{isDeletingUser ? "Deleting..." : "Delete"}</AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog>
        </div>
